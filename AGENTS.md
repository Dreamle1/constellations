...
- The preferred hosted backend on this branch is a Cloudflare Worker in `packages/worker`.
- The Express service in `packages/backend` remains available for local development and Render fallback.
- Shared API types live in `packages/shared`.
- The previous Render backend is available at `https://constellations-3ils.onrender.com`.

## Environment

- Root `.env` is for Expo public frontend variables only.
- `EXPO_PUBLIC_API_URL` should point to the active backend URL.
- Backend secrets such as `OPENAI_API_KEY` must stay in Cloudflare Worker secrets, Render env vars, or `packages/backend/.env`.
- Do not put backend secrets in `EXPO_PUBLIC_*` variables because Expo exposes them to the client.

## Common Commands

- `npm.cmd run type-check` checks TypeScript for the frontend, backend, shared package, and Worker on Windows.
...
- `npm run worker:dev` starts the Cloudflare Worker locally.
- `npm run worker:deploy` deploys the Cloudflare Worker.

## Runtime Notes

- Physical Android devices cannot reach `http://localhost:3001` on the development machine. Use the Worker URL, Render URL, an accessible LAN IP, or an Expo tunnel depending on the scenario.
